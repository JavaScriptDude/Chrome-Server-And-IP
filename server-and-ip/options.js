...
(function (win, doc) {
	'use strict';
	var mnems = JSON.parse(win.sessionStorage.getItem('^mnems^')) || {}, i = 0, ipIndex, more_boxes, mb = 0, mnemBox, sortArr = [],
		sips = JSON.parse(win.sessionStorage.getItem('^sips^')) || {},
	...
	function update_obj (name, obj) {
		try {
			win.sessionStorage.setItem(name, JSON.stringify(obj));
	...
	function removeMnem () {
		...
		// if ip_to_remove exists then remove it from the sessionStorage
	...
	function get_ip_number_in_ls () {
		var ls_c = win.sessionStorage.length;
		ls_c -= (win.sessionStorage.getItem('^mnems^') ? 1 : 0);
		ls_c -= (win.sessionStorage.getItem('^sips^') ? 1 : 0);
		return ls_c;
	}
	...
	// configure and listen to Delete button to clear sessionStorage
	del_ls_btn.textContent = 'Delete ' + get_ip_number_in_ls() + ' items';
	del_ls_btn.addEventListener('click', function (e) {
		var ls_i = 0;
		while (ls_i < win.sessionStorage.length ) {
			if ((win.sessionStorage.key(ls_i) !== '^mnems^') && (win.sessionStorage.key(ls_i) !== '^sips^')) {
				win.sessionStorage.removeItem(win.sessionStorage.key(ls_i));
			} else {
				ls_i += 1;
			}
		}
